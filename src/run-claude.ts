...
export function prepareRunConfig(
  ...
  // Model should be specified in the environment variable ANTHROPIC_MODEL
  // not as a command line argument for claude-code
  ...
  // Add model to environment if specified
  if (options.model) {
    customEnv.ANTHROPIC_MODEL = options.model;
  }
...
export async function runClaude(promptPath: string, options: ClaudeOptions) {
  ...
  const timeoutMs =
    parseInt(process.env.INPUT_TIMEOUT_MINUTES || "30") * 60 * 1000;
